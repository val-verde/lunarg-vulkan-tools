...
MuteMessageWidget::MuteMessageWidget(LayerSetting &layer_setting) : QWidget(nullptr), _layer_setting(layer_setting) {
    ...
    if (!_layer_setting.default_value.isEmpty()) {
        QStringList list = _layer_setting.default_value.split(",");
...
void MuteMessageWidget::addItem(const QString &item) {
    ...
    // Update Setting
    AppendString(_layer_setting.default_value, item);
...
void MuteMessageWidget::removePushed() {
    ...
    // Update Setting
    RemoveString(_layer_setting.default_value, item_name);
    emit itemChanged();
